...
describe('normalizeTrackArtists', () => {
  it('should remove remix from track title and add artist to artists array', () => {
    ...
    expect(normalizedTrack.artists).toEqual(['Alex Lustig', 'Teen Daze']);
    expect(normalizedTrack.results.isRemix).toBe(true);
  });
  ...
  it('should remove exclusive from track title', () => {
    ...
    expect(normalizedTrack.results.isExclusive).toBe(true);
  });
